...
**H1（全幅下線） > H3（短い下線・実質の大見出し） > H2（ドット） > H4（角スクエア）** としている。
...
| — | `<h3>`（CMS本文の実質的な大見出し） | セクションの区切り | Zen Maru Gothic Bold 23px、直下に**短いオレンジ下線**（40×3px）。H1の下線モチーフをスケールダウンしたもの。CMS本文中では上に1pxの罫線＋64pxの余白も加えセクションの切れ目を明示 |
