...
**H1（全幅下線） > H3（全幅下線・実質の大見出し） > H2（ドット） > H4（角スクエア）** としている。
mixin名と`<h2>`〜`<h4>`タグの対応自体は変えず、CSSの中身だけをこの優先度に合わせて割り当てている
（`assets/styles/_mixins.scss` 参照）。
...
| — | `<h2>`（CMS本文ではほぼ未使用。`pages/index.vue`の「記事一覧」等） | 補助的な区切り | Zen Maru Gothic Bold 16px、左に 7px 円のオレンジドット |
| — | `<h4>`（CMS本文の小見出し） | 補足・キャプション見出し | Zen Maru Gothic Bold 15px、左に6px角スクエアのオレンジドット。背景chip化はしない（リストが連続する箇所で見出しが浮いて見えるため。下線も使わない＝H3との混同・リンクとの混同回避） |
