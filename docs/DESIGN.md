...
- コンテンツは「行」ごとではなく**まとまり単位**（ヘッダー・イントロブロック・区切り線・
  記事一覧の見出し・各記事カード・ページネーション・記事詳細ページ全体など）でページ背景色
  （`$color-bg`）と同色の不透明な背景を敷き、パターンを完全に隠す。まとまり同士の余白部分では
  パターンを見せる
- 記事詳細ページ（`pages/article/[id].vue`）は一連の読み物なので、要素ごとではなく`.article`
  全体をひとつの不透明ブロックとして塗る（個別に塗っているのはトップページのカード等、
  グリッド状に独立した要素の場合のみ）
