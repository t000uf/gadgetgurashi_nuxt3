...
<template>
  <div class="sentence_home">
    ...
    <p><nuxt-link to="/article/about">このブログについて</nuxt-link></p>
    <hr>
    <h4 class="headline_home" id="allkiji">記事一覧</h4>
    <!--
    ...
    />
    -->
    <div class="kijilist" id="indexApp">
      <div class="kiji" v-for="content in data?.contents">
        <nuxt-link :to="`/article/${content.id}/`" class="kijiLink"></nuxt-link>
        <h3 class="title">{{content.title}}</h3>
        <div class="thumbPre">
          <img class="thumbnail" v-bind:src="content.thumbnail?.url" v-bind:alt="content.thumbnail.url">
          <p class="preview">{{content.preview}}...</p>
        </div>
        <div class="kijiTags">
          <div class="kijiTag" v-for="(tag, tagkey) in content.tag" :key="tagkey">
			      <nuxt-link :to="`/tag/${tag.id}/page/1/`" class="kijiTagLink"></nuxt-link>
            <p class="kijiTagName">{{tag.tagName}}</p>
          </div>
        </div>
      </div>
    </div>
